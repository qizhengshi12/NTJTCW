   ...
   img {border:0px;}
	...
	<script type="text/javascript">
		function queryMyEmailByBt(){
			...
			form.action = "EmailServlet?action=MyDelEmail&srbt=" + srbt;
			form.submit();
	   	}
	   	function goNum(){
		...
		function skipTo(v){
			...
			form.action = "EmailServlet?action=MyDelEmail&srbt="+srbt+"&page_no=" + v;
			form.submit();
		}
	...
	function deleteSome(){
		...
		if(IDList==""){
			...
			if(confirm("确定删除选中记录么？")){
				...
				form.action = "EmailServlet?action=deleteSomeMyEmail&ly=2&IDList="+IDList;
				form.submit();
			} 
		}
	}
	  ...
	  <table width="100%">
		<tr>
			<td align="left" colspan="2">
				...
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<img  alt="选中" align="middle" src="images/small/favorites.png"><a href='EmailServlet?action=MyDelEmail&flag=1'><span style="font-size: 16px;border:1px; background:#C1FFB2;">回收站</span></a>
			</td>
		</tr>
		<tr >
			<td>
	...
	<table id="table1" width="100%" border=0  cellpadding=0 cellspacing=1 style="">
		...
		<tr class="tr2" onmouseover="this.style.backgroundColor='#EBEBEB';" onmouseOut="this.style.backgroundColor='';">
			...
			<td align="center">
				...
				<a href="EmailServlet?action=ShowEmail&id=<%=MyEmail.getId()%>&sendid=<%=MyEmail.getSendid()%>&zt=<%=MyEmail.getCyzt()%>&ly=2"><%=MyEmail.getBt()%></a>
				<%}else{ %>
				&nbsp;
				<%} %>
			</td>
			...
			<td align="center">
				...
				<a href="EmailServlet?action=ShowEmail&id=<%=MyEmail.getId()%>&sendid=<%=MyEmail.getSendid()%>&zt=<%=MyEmail.getCyzt()%>&ly=2"><img  alt="未查阅" title="未查阅" src="images/small/envelope1.png"></a>
				<%}else{ %>
				<a href="EmailServlet?action=ShowEmail&id=<%=MyEmail.getId()%>&sendid=<%=MyEmail.getSendid()%>&zt=<%=MyEmail.getCyzt()%>&ly=2"><img  alt="已查阅" title="已查阅" src="images/small/envelope.png"></a>
				<%} %>
			</td>
			<td align="center">
				<a href="EmailServlet?action=returnMyEmail&id=<%=MyEmail.getId()%>"><img  alt="还原" title="还原" src="images/small/redo.png"></a>
				<a href="EmailServlet?action=deleteMyEmail&id=<%=MyEmail.getId()%>&ly=2" onclick="javascript:return confirm('确定删除此记录？');"><img  alt="删除" title="删除" src="images/small/burn.png"></a>
			</td>
		</tr>
		<%
		}
