	...
	<body>
		...
		<input type="hidden" name="FileUrl" id="FileUrl" value="<%=Glzd.getFileUrl()%>">
			<table  width="100%">
				<tr>
					<td>
					标题：<input name="bt" id="bt" type="text" value="<%=Glzd.getBt()%>"/>
					文号：<input name="wh" id="wh" type="text" value="<%=Glzd.getWh()%>"/>
					实施日期：<input name="ssrq" id="ssrq" type="text" value="<%=Glzd.getSsrq()%>" size="10" readonly/>
          <input name="Button" class="button1" onclick="setDay(document.Glzd.ssrq);" type="button" value="选择">
					<input type="radio" name="sfgk"  <%if("0".equals(Glzd.getSfgk())){%>checked="checked"<%}%>  value="0">对外公开
					<input type="radio" name="sfgk"  <%if("1".equals(Glzd.getSfgk())){%>checked="checked"<%}%>  value="1">单位内部
					</td>
				</tr>
				<tr>
					<td>
