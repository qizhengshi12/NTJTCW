  ...
  <head>
   ...
   <script src="calendar.js"></script>
	...
	<script type="text/javascript">
    	...
    	<% if(UserPer.getTjbb().indexOf("5")!=-1){ %>
		...
		function query(menuname,str){
			...
			if(str==1){
				form.action = "BbsbTjbbServlet?action=getTjbbYxzbSix&flag=1&menuname="+menuname;
				form.submit();
			}else if(str==2){
				...
				form.action = "BbsbTjbbServlet?action=getTjbbYxzbHsZdhz&flag=1&menuname="+menuname;
				form.submit();
			}else if(str==6){
				...
				form.action = "BbdzServlet?action=getBbdz&flag=1&lx=tjbb&zbid="+zbid+"&menuname="+menuname;
				form.submit();
			}
		...
		function resetMenu(){
			...
			form.action = "BbsbTjbbServlet?action=resetMenu";
			form.submit();
		}
	</script>
  </head>
			...
			<td width="15%" valign="top"  class="leftmenu">
				<div id=treeviewarea>
					<script language="javascript">
				    	...
				    	tree.icons["wh"]  = "wh.png"; //上月内容未提交时显示
					...
					<td height="35px">
						统计报表：
					  	<select id="sel1" onchange="selChange1()" class="STYLE1">
							<option value=""></option>
							<option value="1">主要运行指标统计表（六县市、运管处）</option>
							<%if(("统计负责人".equals(roles)&&"局机关".equals(company))||("航道管理处".equals(company))){%>
							<option value="2">主要运行指标统计表（航道处）</option>
							<%} %>
							<%if(("统计负责人".equals(roles)&&"局机关".equals(company))||("汽运实业集团有限公司".equals(company))){%>
							<option value="3">客运出站运输量统计表（汽运集团）</option>
							<%} %>
							<%if(("统计负责人".equals(roles)&&"局机关".equals(company))||("地方海事局".equals(company))){%>
							<option value="4">海事站点签证数据（海事局）</option>
							<option value="5">海事站点签证数据汇总表（海事局）</option>
							<option value="6">海事签证月度同比表（海事局）</option>
							<%} %>
							<%
					         	/*遍历每个的信息进行显示*/
