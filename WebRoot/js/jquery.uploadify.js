    ...
    var D = "undefined", r = "object", S = "Shockwave Flash", W = "ShockwaveFlash.ShockwaveFlash", q = "application/x-shockwave-flash", R = "SWFObjectExprInst", x = "onreadystatechange", O = window, j = document, t = navigator, T = false, U = [h], o = [], N = [], I = [], l, Q, E, B, J = false, a = false, n, G, m = true, M = function () {
        ...
        a = true; E = Z || null; B = { success: false, id: X }; var ae = c(X); if (ae) {
            if (ae.nodeName == "OBJECT") { l = g(ae); Q = null; } else { l = ae; Q = X; } aa.id = R; if (typeof aa.width == D || (!/%$/.test(aa.width) && parseInt(aa.width, 10) < 310)) {
                aa.width = "310";
            } if (typeof aa.height == D || (!/%$/.test(aa.height) && parseInt(aa.height, 10) < 137)) { aa.height = "137"; } j.title = j.title.slice(0, 47) + " - Flash播放器安装"; //XTing Flash Player Installation
...
(function ($) {
            ...
            return this.each(function () {
                ...
                if (flashInstalled) {
                    ...
                    swfuploadify.queueData = {
                        ...
                        errorMsg: '一些文件没有添加到队列中:'//XTing Some files were not added to the queue
                    };
                ...
                if (args[0]) {
                    ...
                    if (args[0] == '*') {
                        ...
                        $('#' + settings.queueID).find('.uploadify-queue-item').each(function () {
                            ...
                            $(this).find('.data').removeClass('data').html(' - 取消上传');//XTing Cancelled
                        ...
                        for (var n = 0; n < args.length; n++) {
                            ...
                            $('#' + args[n]).find('.data').removeClass('data').html(' - 取消上传');//XTing Cancelled
                    ...
                    $item.find('.data').removeClass('data').html(' - 取消上传');//XTing Cancelled
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '存在受限制文件，无法添加到队列当中:';//XTing Some files were not added to the queue
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('该文件 "' + file.name + '" 已经存在队列中了。\n你想把已存在队列中的项目替换掉吗?');
                    //XTing var replaceQueueItem = confirm('The file named "' + file.name + '" is already in the queue.\nDo you want to replace the existing item in the queue?');
                    if (!replaceQueueItem) {
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            this.queueData.errorMsg += '\n所选文件的数量超过剩余的上传限制 (' + errorMsg + ').';//XTing The number of files selected exceeds the remaining upload limit
                        } else {
                            this.queueData.errorMsg += '\n所选文件的数量超过了队列大小限制 (' + settings.queueSizeLimit + ').';//XTing The number of files selected exceeds the queue size limit
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n该文件 "' + file.name + '" 超过了上传大小限制 (' + settings.fileSizeLimit + ').';//XTing exceeds the size limit
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n该文件 "' + file.name + '" 是空文件。';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n该文件 "' + file.name + '" 的文件类型不允许上传 (' + settings.fileTypeDesc + ').';//XTing is not an accepted file type
                        break;
                }
            }
        ...
        onUploadError: function (file, errorCode, errorMsg) {
            ...
            var errorString = '错误';//XTing Error
            switch (errorCode) {
                case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
                    errorString = 'HTTP 错误 (' + errorMsg + ')';//XTing
                    break;
                case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
                    errorString = '上传路径不存在';//XTing Missing Upload URL
                    break;
                case SWFUpload.UPLOAD_ERROR.IO_ERROR:
                    errorString = 'IO 错误';//XTing
                    break;
                case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
                    errorString = '安全错误';//XTing Security Error
                    break;
                case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                    alert('已经达到了上传限制 (' + errorMsg + ').');//XTing The upload limit has been reached
                    errorString = '超过了上传限制';//XTing Exceeds Upload Limit
                    break;
                case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
                    errorString = '失败';//XTing Failed
                    break;
                case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
                    break;
                case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
                    errorString = '验证错误';//XTing Validation Error
                    break;
                case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
                    errorString = '取消';//XTing Cancelled
                    this.queueData.queueSize -= file.size;
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = confirm('该文件 "' + file.name + '" 在服务器中存在同名文件.\n您要替换同名文件吗?');
                            //XTing var overwrite = confirm('A file with the name "' + file.name + '" already exists on the server.\nWould you like to replace the existing file?');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
        ...
        onUploadSuccess: function (file, data, response) {
            ...
            if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
                $('#' + file.id).find('.data').html(' - 上传成功');//XTing Complete
            }
    ...
    $.fn.uploadify = function (method) {
        ...
        if (methods[method]) {
            ...
            $.error('该方法 ' + method + ' 不存在 $.uploadify中');
            //XTing $.error('The method ' + method + ' does not exist in $.uploadify');
        }
